    ...
    bot.commands = {
        ...
        stop: msg => {
            ...
            if(track && track.dispatcher && msg && msg.channel) {
                ...
                track.dispatcher.end();
                bot.paused = false;
                msg.channel.sendMessage(`:stop_button: "${track.title}" Durduruldu`);
            }
        },

        list: msg => {
            var list = __.map(bot.queue.list, (track, idx) => `${idx + 1}. Type: "${track.type}" Title: "${track.title}${track.requestor ? ` Talep eden kişi: ${track.requestor}`:''}"`);
            if(list.length > 0)
                msg.channel.sendMessage(list.join('\n'));
            else
                msg.channel.sendMessage(':cd: Sırada Herhangi bir şarkı yok.');
        },

        clear: msg => {
            bot.commands.stop(msg);
            bot.queue.clear();
            msg.channel.sendMessage(':cd: Oynatma Listesi Temizlendi');
        },
        ...
        volume: msg => {
            ...
            if(volume != '') {
                ...
                msg.channel.sendMessage(`:speaker: Ayarlanacak Ses : ${volume * 100}%`);
            } else
                msg.channel.sendMessage(`:speaker: Ayarlanan Ses : ${bot.config.stream.volume * 100}%`);
        },

        repeat: msg => {
            bot.config.queue.repeat = !bot.config.queue.repeat;
            msg.channel.sendMessage(`Yeniden Oynatma ${bot.config.queue.repeat ? 'Açık' : 'Kapalı'}`);
        },
